...
describe 'deployments#index' do
  ...
  before(:each) do
    ...
    @huddle.deployments << @deployment_1 << @deployment_2
    @huddle.save

    @polaris.deployments << @deployment_3
    @polaris.save
  ...
  context 'when viewing deployments for all projects' do
  ...
  context 'when viewing deployments for a specific project' do
    ...
    it 'displays the list of deployments for this project only' do
      ...
      page.should have_content('1/2')
      within_table('deployments-2011-01-02') do
        page.should_not have_xpath("./tr[1][@id='deployment-#{@deployment_2.id}']")
